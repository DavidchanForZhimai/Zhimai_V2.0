...
@interface DWEqualSpaceFlowLayout : UICollectionViewFlowLayout
...
@implementation DWEqualSpaceFlowLayout
...
- (void)setCell:(DWTagCell *)cell selected:(BOOL)selected {
...
- (UICollectionView *)collectionView {
    if (!_collectionView) {
        DWEqualSpaceFlowLayout *flowLayout = [[DWEqualSpaceFlowLayout alloc] init];
        flowLayout.delegate = self;
        _collectionView = [[UICollectionView alloc] initWithFrame:self.bounds collectionViewLayout:flowLayout];
        _collectionView.backgroundColor = [UIColor clearColor];
        [_collectionView registerClass:[DWTagCell class] forCellWithReuseIdentifier:kTagCellID];
...
- (NSMutableArray<DWTagModel *> *)tagModels {
    if (!_tagModels) {
