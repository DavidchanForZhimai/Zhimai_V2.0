...
+ (NSMutableArray*)getSearchResultBySearchText:(NSString*)searchText dataArray:(NSMutableArray*)dataArray
{
    ...
    if (searchText.length>0&&![ChineseInclude isIncludeChineseInString:searchText]) {
        for (int i=0; i<dataArray.count; i++) {
            ...
            if ([ChineseInclude isIncludeChineseInString:dataArray[i][@"area"]]) {
                NSString *tempPinYinStr = [PinYinForObjc chineseConvertToPinYin:dataArray[i][@"area"]];
                ...
                NSString *tempPinYinHeadStr = [PinYinForObjc chineseConvertToPinYinHead:dataArray[i][@"area"]];
                ...
                NSRange titleResult=[dataArray[i][@"area"] rangeOfString:searchText options:NSCaseInsensitiveSearch];
                if (titleResult.length>0) {
                    [searchResults addObject:dataArray[i]
                     ];
                }
        ...
        for (NSDictionary *tempStr in dataArray) {
            NSRange titleResult=[tempStr[@"area"] rangeOfString:searchText options:NSCaseInsensitiveSearch];
            if (titleResult.length>0) {
