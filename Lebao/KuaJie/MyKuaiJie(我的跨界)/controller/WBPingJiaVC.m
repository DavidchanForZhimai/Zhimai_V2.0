...
@interface WBPingJiaVC ()<UITextViewDelegate>

@end

@implementation WBPingJiaVC

- (void)viewDidLoad {
    [super viewDidLoad];
    [self setNav];
    self.view.backgroundColor = BACKCOLOR;
    self.sfmyLab.text = _pjStr;
...
- (IBAction)quedingAction:(id)sender {
    if ([_pyText.text length]<5 || [_pyText.text length] > 50||[_pyText.text isEqualToString:@"请输入5-50个字的评语"]) {
        [[ToolManager shareInstance]showAlertMessage:@"请输入5-50个字的评语"];
        return;
    }
    int scor;
    if ([_pjStr isEqualToString:@"满意"]) {
        scor = 1;
    }else
    {
        scor = -1;
    ...
    if (_type == 1) {
        //发布评价
    [[MyKuaJieInfo shareInstance]pingJiaWithID:_xiansuoID andScord:scor andContent:_pyText.text andType:_type andCallBack:^(BOOL issucced, NSString *info, NSDictionary *jsonDic) {
            if (issucced == YES) {
