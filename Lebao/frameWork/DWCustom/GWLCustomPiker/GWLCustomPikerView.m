...
- (void)configureToolView {
    ...
    [cancelButton setTitleColor:AppMainColor forState:UIControlStateNormal];
    [cancelButton addTarget:self action:@selector(cancelButtonDidClick:) forControlEvents:UIControlEventTouchDown];
...
- (void)cancelButtonDidClick:(UIButton *)sender{
    ...
    [UIView animateWithDuration:0.5 animations:^{
        ...
        self.frame = CGRectMake(0, APPHEIGHT , APPWIDTH, 220);
    }];
}
...
- (void)layoutSubviews {
    ...
    _titleButton.width                      =  _cancelButton.width = 44;
    _titleButton.height                     = _cancelButton.height = _toolView.height;
    _titleButton.X                          = _toolView.width - margin - _titleButton.width;
    
    _cancelButton.X                        = margin;
    
    _titleLabel.X                           = 3*margin/2.0 + _cancelButton.width ;
    _titleLabel.size                        = CGSizeMake(self.width-_titleButton.width -_cancelButton.width-margin*3, _titleButton.height);
