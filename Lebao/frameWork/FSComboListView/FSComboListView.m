...
#define COMBOBOX_HEIGHT 25.0
#define COMBOBOX_MARGIN 50.0
...
- (void)initialize
{
    ...
    _pickerView = [[UIPickerView alloc] initWithFrame:CGRectMake(0, 0, APPWIDTH - COMBOBOX_MARGIN*2, PICKER_VIEW_HEIGHT)];
...
//    [self addSubview:_pickerView];
    ...
    the_label.textColor = BlackTitleColor;
    the_label.text = [_values objectAtIndex:0];
    the_label.font = [UIFont systemFontOfSize:28*SpacedFonts];
    the_label.textAlignment = NSTextAlignmentCenter;
    [self addSubview:the_label];
    [the_label.layer setBorderColor:LineBg.CGColor];
    [the_label.layer setBorderWidth:0.5];
...
- (id)initWithValues:(NSArray *)values frame:(CGRect)frameRect
{
    ...
    if (self) {
        ...
        for (NSString* option in _values) {
            ...
            CGRect rect = [option boundingRectWithSize:CGSizeMake(MAXFLOAT, 25)
                                                                options:NSStringDrawingUsesLineFragmentOrigin
                                                             attributes:@{NSParagraphStyleAttributeName: paragraphStyle,NSFontAttributeName: [UIFont systemFontOfSize:24*SpacedFonts]}
                                                                context:nil];
...
{
    return [UILabel createLabelWithFrame:frame(0, 0, frameWidth(pickerView), COMBOBOX_HEIGHT) text:[_values objectAtIndex:row] fontSize:28*SpacedFonts textColor:BlackTitleColor textAlignment:NSTextAlignmentCenter inView:nil];
}
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component
{
    ...
    if (_changedBlock) {
        _changedBlock(self,[_values objectAtIndex:row]);
    }
    [self dismiss];
}
