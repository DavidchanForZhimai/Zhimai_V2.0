...
@property (nonatomic) NSNumber *StartFromThisTab;
...
@synthesize StartFromThisTab = _StartFromThisTab;
...
- (void)layoutSubviews {
    ...
    frame.origin.x = 0.0;
    frame.origin.y = self.tabsView.frame.origin.y+ CGRectGetHeight(self.tabsView.frame);
    frame.size.width = CGRectGetWidth(self.view.frame);
...
- (void)setStartFromThisTab:(NSNumber *)StartFromThisTab {
    _StartFromThisTab = StartFromThisTab;
}
...
- (NSNumber *)StartFromThisTab {
    
    if (!_StartFromThisTab) {
        CGFloat value = kStartFromThisTab;
        if ([self.delegate respondsToSelector:@selector(viewPager:valueForOption:withDefault:)])
            value = [self.delegate viewPager:self valueForOption:ViewPagerOptionStartFromThisTab withDefault:value];
        self.StartFromThisTab = [NSNumber numberWithInteger:value];
    }
    return _StartFromThisTab;
}
...
- (CGFloat)valueForOption:(ViewPagerOption)option {
    ...
    switch (option) {
        ...
        case ViewPagerOptionStartFromThisTab:
            return [[self StartFromThisTab] integerValue];
...
- (void)setIspageViewController:(BOOL)ispageViewController
{
    if (ispageViewController) {
...
- (void)defaultSetup {
    ...
    // Select starting tab
    [self selectTabAtIndex:[self.StartFromThisTab integerValue]];
...
#pragma mark
#pragma mark 改变顶栏标题颜色
- (void)changeTitleColor
{
    for (UIView *tabView1 in _tabs)
    {
        for (id sender in tabView1.subviews)
        {
            [sender isKindOfClass:[UILabel class]];
            
            UILabel *label = (UILabel *)sender;
            label.textColor = LightBlackTitleColor;
        }
    }
    
    UIView *tabView = [self tabViewAtIndex:self.activeTabIndex];
    for (id sender in tabView.subviews)
    {
        [sender isKindOfClass:[UILabel class]];
